...
async function getGenerators({
  ...
  try {
    const generators = await pMap(generatorConfigs, async (generator, index) => {
      ...
      if (generator.output) {
        ...
        if (!generatorInstance.manifest || !generatorInstance.manifest.defaultOutput) {
          throw new Error(`Can't resolve output dir for generator ${chalk.bold(generator.name)} with provider ${chalk.bold(generator.provider)}.
The generator needs to either define the \`defaultOutput\` path in the manifest or you need to define \`output\` in the datamodel.prisma file.`);
      ...
      const options = {
        ...
        schemaPath,
        version: version || enginesVersion
      };
    ...
    const generatorProviders = generatorConfigs.map((g) => parseEnvValue(g.provider));
...
const knownBinaryTargets = [...platforms, "native"];
...
Please use the PRISMA_QUERY_ENGINE_BINARY env var instead to pin the binary target.`);
    ...
    if (generator.binaryTargets) {
      ...
      const resolvedBinaryTargets = binaryTargets.flatMap((object) => parseBinaryTargetsEnvValue(object)).map((p) => p === "native" ? platform2 : p);
      for (const resolvedBinaryTarget of resolvedBinaryTargets) {
        if (oldToNewBinaryTargetsMapping[resolvedBinaryTarget]) {
          throw new Error(`Binary target ${chalk.red.bold(resolvedBinaryTarget)} is deprecated. Please use ${chalk.green.bold(oldToNewBinaryTargetsMapping[resolvedBinaryTarget])} instead.`);
        }
        if (!knownBinaryTargets.includes(resolvedBinaryTarget)) {
          throw new Error(`Unknown binary target ${chalk.red(resolvedBinaryTarget)} in generator ${chalk.bold(generator.name)}.
Possible binaryTargets: ${chalk.greenBright(knownBinaryTargets.join(", "))}`);
      ...
      if (!resolvedBinaryTargets.includes(platform2)) {
        ...
        if (generator) {
          console.log(`${chalk.yellow("Warning:")} Your current platform \`${chalk.bold(platform2)}\` is not included in your generator's \`binaryTargets\` configuration ${JSON.stringify(originalBinaryTargetsConfig)}.
    To fix it, use this generator config in your ${chalk.bold("schema.prisma")}:
    ${chalk.greenBright(printGeneratorConfig({
            ...generator,
            binaryTargets: fixBinaryTargets(generator.binaryTargets, platform2)
          }))}
    ${chalk.gray(`Note, that by providing \`native\`, Prisma Client automatically resolves \`${platform2}\`.
    Read more about deploying Prisma Client: ${chalk.underline("https://github.com/prisma/prisma/blob/main/docs/core/generators/prisma-client-js.md")}`)}
`);
        } else {
          console.log(`${chalk.yellow("Warning")} The binaryTargets ${JSON.stringify(originalBinaryTargetsConfig)} don't include your local platform ${platform2}, which you can also point to with \`native\`.
    In case you want to fix this, you can provide ${chalk.greenBright(`binaryTargets: ${JSON.stringify(["native", ...binaryTargets || []])}`)} in the schema.prisma file.`);
        }
      }
    }
  }
