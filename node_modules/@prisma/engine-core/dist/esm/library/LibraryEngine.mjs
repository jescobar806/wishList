...
You may have to run ${chalk.greenBright("prisma generate")} for your changes to take effect.`, this.config.clientVersion);
  ...
  prismaGraphQLToJSError(error) {
    ...
    if (error.user_facing_error.error_code) {
      return new PrismaClientKnownRequestError(error.user_facing_error.message, error.user_facing_error.error_code, this.config.clientVersion, error.user_facing_error.meta);
    }
  ...
  async requestBatch(queries, headers = {}, transaction = false, numTry = 1) {
    ...
    if (Array.isArray(batchResult)) {
      return batchResult.map((result2) => {
        if (result2.errors) {
          return this.loggerRustPanic ?? this.prismaGraphQLToJSError(result2.errors[0]);
        }
        return {
          data: result2,
          elapsed: 0
